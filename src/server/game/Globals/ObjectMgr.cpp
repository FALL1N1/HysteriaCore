...
void ObjectMgr::LoadLocationScripts()
{
    ...
    if (!result)
    {
        sLog->outString(">> Loaded 0 wold location scripts. DB table `location_scripts` is empty.");
        return;
    }
    ...
    sLog->outString(">> Loaded %u location scripts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}

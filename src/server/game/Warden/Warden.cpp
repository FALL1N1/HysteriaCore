    ...
    if (_initialized)
    {
        ...
        if (_dataSent)
        {
            ...
            if (maxClientResponseDelay > 0)
            {
                ...
                {
                    //sLog->outString("[WARDEN ANTICHEAT] Player %s got kicked for being idle for more than %s minutes!", _session->GetPlayerName(), maxClientResponseDelay.c_str());
                    _session->KickPlayer();
                }
                else
                {
...
std::string Warden::Penalty(WardenCheck* check /*= NULL*/, uint16 checkFailed /*= 0*/)
{
    ...
    case WARDEN_ACTION_LOG:
        //sLog->outString("[WARDEN ANTICHEAT][HACKING] Player %s got caught hacking\"%s\", please check him!", _session->GetPlayerName(), banReason.c_str());
        return "None";
        break;
    case WARDEN_ACTION_KICK:
        /*if (checkFailed)
            sLog->outString("[WARDEN ANTICHEAT][HACKING] Player %s got kicked for \"%s\"", _session->GetPlayerName(), banReason.c_str());
        else
            sLog->outString("[WARDEN ANTICHEAT][HACKING] Player %s got kicked. No reason, something custom?", _session->GetPlayerName().c_str());
        */
        _session->KickPlayer();
        return "Kick";
        break;

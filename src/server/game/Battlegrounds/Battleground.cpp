...
void Battleground::Update(uint32 diff)
{
    ...
    // Anti-Draw (Dementia)
    if (sWorld->getBoolConfig(ARENA_DEMENTIA_ENABLED))
    {
                    ...
                    if (!player->IsSpectator() && !player->IsGameMaster())
                    {
                        if ((GetStartTime() >= 10 * MINUTE * IN_MILLISECONDS) && (GetStartTime() <= 10.1 * MINUTE * IN_MILLISECONDS))
                        {
                            if (!player->HasAura(41406))
                            {
                                ...
                                player->GetSession()->SendAreaTriggerMessage("Damage and healing is going to be progressively increased every minute from now! This is made in case of draw for this arena match.");
                            }
                        ...
                        if ((GetStartTime() >= 14 * MINUTE * IN_MILLISECONDS) && (GetStartTime() <= 14.1 * MINUTE * IN_MILLISECONDS))
                        {
                            ...
                            {
                                player->AddAura(41406, player);
                                player->GetSession()->SendAreaTriggerMessage("Damage and healing is increased! This is the last tick of anti-draw system which means damage and healing won't be increased anymore!");
                            }

                ...
                if (Map* map = owner->FindMap())
                    if (Battleground* bg = ((BattlegroundMap*)map)->GetBG())
                    {
                        ...
                        if (GameObject* pillar = ((BattlegroundRV*)bg)->GetPillarAtPosition(&dest))
                        {
                            ...
                            {
                                //Position pos = owner->GetFirstCollisionPositionForTotem(/* init pos here */, owner->GetExactDist2d(i_target.getTarget()), owner->GetAngle(i_target.getTarget()) - owner->GetOrientation(), false);
                                //x = pos.GetPositionX();
                                //y = pos.GetPositionY();
                                //z = 28.28f;
                            }

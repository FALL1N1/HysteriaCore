...
void ChaseMovementGenerator<Creature>::MovementInform(Creature* owner)
{
    ...
    if (owner->AI())
        owner->AI()->MovementInform(CHASE_MOTION_TYPE, GetTarget()->GetGUIDLow());
}
...
void FollowMovementGenerator<Creature>::UpdateSpeed(Creature* owner)
{
    ...
    owner->UpdateSpeed(MOVE_WALK, false);
    owner->UpdateSpeed(MOVE_SWIM, false);
}
...
void FollowMovementGenerator<Creature>::MovementInform(Creature* unit)
{
    // Pass back the GUIDLow of the target. If it is pet's owner then PetAI will handle
    if (unit->AI())
        unit->AI()->MovementInform(FOLLOW_MOTION_TYPE, GetTarget()->GetGUIDLow());
}
